...
describe('index - fetchSettings', () => {
  ...
  afterEach(() => {
    ...
    delete import.meta.env.VITE_APP_INVENTORY_MANAGEMENT_SYSTEM_BUILD_DIRECTORY;
  });
  ...
  it('settings are loaded', async () => {
    ...
    server.use(
      http.get('/inventory-management-system-settings.json', () =>
  ...
  it('logs an error if API URLs is not defined in the settings', async () => {
    ...
    expect(mockLog.calls[0][0]).toEqual(
      'Error loading /inventory-management-system-settings.json: apiUrl is undefined in settings'
    );
  });
  ...
  it('logs an error if settings.json is an invalid JSON object', async () => {
    ...
    const mockLog = vi.mocked(log.error).mock;
    expect(mockLog.calls[0][0]).toEqual(
      'Error loading /inventory-management-system-settings.json: Invalid format'
    );
  });
  ...
  it('logs an error if settings.json fails to be loaded with custom path', async () => {
    ...
    const mockLog = vi.mocked(log.error).mock;
    expect(mockLog.calls[0][0]).toEqual(
      'Error loading /custom/directory/inventory-management-system-settings.json: Request failed with status code 404'
    );
  });
  ...
  it('logs an error if fails to load a settings.json and is still in a loading state', async () => {
    ...
    const mockLog = vi.mocked(log.error).mock;
    expect(mockLog.calls[0][0]).toEqual(
      'Error loading /inventory-management-system-settings.json: Request failed with status code 500'
    );
  });
  ...
  it('logs an error if no routes are defined in the settings', async () => {
    ...
    const mockLog = vi.mocked(log.error).mock;
    expect(mockLog.calls[0][0]).toEqual(
      'Error loading /inventory-management-system-settings.json: No routes provided in the settings'
    );
  });
  ...
  it('logs an error if route has missing entries', async () => {
    ...
    const mockLog = vi.mocked(log.error).mock;
    expect(mockLog.calls[0][0]).toEqual(
      'Error loading /inventory-management-system-settings.json: Route provided does not have all the required entries (section, link, displayName)'
    );
  });
});
